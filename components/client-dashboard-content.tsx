...
export function ClientDashboardContent({ activeTab }: { activeTab: string }) {
  return (
    <div className="flex flex-col min-h-full w-full">
      <Header />
      <div className="flex-1 p-4 md:p-6 lg:p-8 max-w-[1400px] mx-auto w-full">
        {activeTab === "Home" && <ClientHomeView />}
        {activeTab === "Tasks" && <TaskView userType="client" />}
        {activeTab === "Calendar" && <CalendarView />}
        {activeTab !== "Home" && activeTab !== "Tasks" && activeTab !== "Calendar" && (
          <div className="flex flex-col items-center justify-center h-full text-center py-24">
            ...
            <p className="text-muted-foreground italic max-w-xs mt-2">
              This section is coming soon for your client portal.
            </p>
          </div>
        )}
      </div>
